...
const removeLeadingTime = (value) => value.replace(/^\s*(?:[🕘🕙🕚🕛🕐🕑🕒🕓🕔🕕🕖🕗]\s*)?(?:[01]?\d|2[0-3]):[0-5]\d(?:\s*(?:-|–|—|~|〜|至|到)\s*(?:[01]?\d|2[0-3]):[0-5]\d)?\s*[:：-]?\s*/, '');
const normalizeHourCode = (value) => {
    const number = typeof value === 'number' ? value : Number(value);
    return Number.isFinite(number) ? `H${String(number).padStart(3, '0')}` : '';
};
const extractHourRange = (value) => {
...
const parseHourHeading = (value) => {
    ...
    if (suffix && /^[-–—~〜至到]\s*H?\s*\d{1,3}(?:$|[\s:：、，,|｜])/.test(suffix))
        return null;
    const label = cleanMarkdown(suffix
        .replace(/^[:：、，,.。|｜·•—–\-]+\s*/, '')
        .replace(/^（\s*/, '')
        .replace(/^\(\s*/, '')
        .replace(/\s*[）)]$/, ''));
...
const createTask = (titleValue, completed, contextTime, note) => {
    ...
    if (title.length < 2 || title.length > 240 || /^[-—–|]+$/.test(title))
        return null;
...
const parseTableRow = (line, contextTime, note) => {
...
export function parseMarkdown(content, filename) {
    ...
    const addTask = (task) => {
        ...
        if (seen.has(key))
            return;
        seen.add(key);
        if (tasks.length < MAX_TASKS_PER_FILE)
            tasks.push(task);
    };
    const addHourDetail = (task, value) => {
        if (!task)
            return;
        ...
        if (!detail || /^[-—–|]+$/.test(detail))
            return;
        if (!task.details.includes(detail) && task.details.length < MAX_DETAILS_PER_TASK)
    ...
    lines.forEach((rawLine) => {
        ...
        if (/^>/.test(line)) {
            ...
            if (lastTask) {
                ...
                if (detail && !lastTask.details.includes(detail) && lastTask.details.length < MAX_DETAILS_PER_TASK)
                    lastTask.details.push(detail);
            }
            return;
        }
