...
export function repeatMatches(repeat: RepeatRule, weekday: Weekday): boolean {
...
export function describeRepeat(repeat: RepeatRule): string {
  ...
  if (!repeat.days.length) return '未选择日期'
  const ordered = [...new Set(repeat.days)].sort((a, b) => a - b)
  if (ordered.length === 7) return '每天'
  if (ordered.length === 5 && ordered.every((d) => d >= 1 && d <= 5)) return '工作日'
  if (ordered.length === 2 && ordered.includes(0) && ordered.includes(6)) return '周末'
...
export function generateDailyTasks(
  ...
  const generated: QueueTask[] = []
  templates.forEach((template) => {
    if (!repeatMatches(template.repeat, weekday)) return
